Host// Copyright The OpenTelemetry Authors
//
...
func (r *purefaScraper) scrape(_ context.Context) (pmetric.Metrics, error) {
	...
	for _, stats := range allServerStats {
		for k, v := range stats.Stats {
			switch k {
			case "host_connections_info":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostPerformanceAverageBytesDataPoint(now, parsedV)
				}
			case "host_performance_average_bytes":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostPerformanceBandwidthByteshDataPoint(now, parsedV)
				}
			case "host_performance_bandwidth_bytes":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostPerformanceLatencyUsecDataPoint(now, parsedV)
				}
			case "host_performance_latency_usec":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostPerformanceThroughputIopsDataPoint(now, parsedV)
				}
			case "host_performance_throughput_iops":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostSpaceBytesDataPoint(now, parsedV)
				}
			case "host_space_byte":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostSpaceDataReductionRatioDataPoint(now, parsedV)
				}
			case "host_space_data_reduction_ratio":
				if parsedV, ok := r.parseInt(k, v); ok {
					r.mb.RecordPurefaHostComponentTemperatureCelsiusDataPoint(now, parsedV)
				}
			}
		}
	}
